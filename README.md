...
Professor ratings, grade distributions, and detailed profiles, shown right where you browse UCSC courses on MyUCSC and Schedule Planner.
...
Ratings in Schedule Planner, right beside the instructor in your Current Schedule and in the section picker — the screen where you choose between lecture sections.

![Schedule Planner with inline ratings in the Current Schedule table](assets/screenshot-scheduler-home.png)

On a generated schedule, each section gets the full rating bar, and every calendar block carries the professor's score. The schedule summary table lists no instructor at all, so those names are resolved from Schedule Planner's own section data.
...
- **Schedule Planner support.** Ratings appear in the Current Schedule table, the section picker, generated schedules, and calendar blocks.
...
- **Accurate matching.** MyUCSC abbreviates instructors to "Lee,D.", which two different professors can share. Ratings are matched using the course subject to tell them apart, and on Schedule Planner the instructor's CruzID is read directly from the page's own data, so no guessing is involved.
...
The same bar appears in Schedule Planner, sized to fit each surface: a compact badge beside the instructor's name in a table, the full bar beneath a section on a generated schedule, and a bare score inside a calendar block.
...
- **Schedule Planner content script** runs on `ucsc.collegescheduler.com`. Kept separate because the two hosts have little in common: one is PeopleSoft with full page loads and abbreviated names, the other a React SPA that never reloads, hashes its own CSS class names, and exposes exact instructor identities over JSON.
...
`public/data/instructors.json` maps CruzIDs to instructor names and the subjects they teach, and is what disambiguates professors who share a `Last,F.` key. It is generated from Schedule Planner's section data, which publishes each instructor's campus email:

```bash
# 1. Sign in and open Schedule Planner, then paste this into the devtools console.
...
- On Schedule Planner, section data is read from that site's own API, same-origin, using your existing session — the same requests the page already makes to render itself. Nothing is sent anywhere else, and no credentials ship with the extension.
